  ...
  export default {
    ...
    methods: {
      ...
      register() {
        ...
        if(valid){
          credentials.name = this.model.name;
          credentials.email = this.model.email;
          credentials.company_name = this.model.company_name;
...
// Post to server
          axios.post("http://localhost:3128/register", credentials).then(res => {
      ...
      login() {
        ...
        credentials.email = this.model.email;
        credentials.password = this.model.password;
        // console.log(formData, this.model.email, this.model.password);
        this.loading = "Signing in";
        // Post to server
        axios.post("http://localhost:3128/login", credentials).then(res => {
          // Post a status message
          this.loading = "";
          if (res.data.status == true) {
            // now send the user to the next route
